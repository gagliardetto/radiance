...
func (mr *MemoryRegion) VmToHost(vmAddr u64_, len uint64) (unsafe.Pointer, EbpfError) {
	...
	if vmAddr.u64() < mr.VmAddr {
		return nil, &InvalidVirtualAddress{vmAddr.u64()}
	}
	...
	if endOffset := gappedOffset + len; endOffset <= mr.Len && !isInGap {
		...
		return unsafe.Add(mr.HostAddr, gappedOffset), nil
	}
	return nil, &InvalidVirtualAddress{vmAddr.u64()}
}
...
func (mr MemoryRegion) String() string {
	return fmt.Sprintf(
		...
		mr.HostAddr,
		u64_(uintptr(mr.HostAddr)).saturating_add(mr.Len),
		mr.VmAddr,
...
func (m *MemoryMapping) Map(accessType AccessType, vm_addr uint64, length uint64) (unsafe.Pointer, error) {
	index := vm_addr >> VIRTUAL_ADDRESS_BITS
	if index >= uint64(len(m.regions)) {
		return nil, &InvalidMemoryRegion{index}
	}
...
func (m *MemoryMapping) GenerateAccessViolation(accessType AccessType, vm_addr uint64, len uint64) (unsafe.Pointer, error) {
	...
	if !m.config.DynamicStackFrames && stack_frame >= 0 && stack_frame <= uint64(m.config.MaxCallDepth)+1 {
		return nil, &StackAccessViolation{
			0, // Filled out later
			accessType,
			vm_addr,
			len,
